...
model.load_state_dict(torch.load('saved-models/model_humidity.pth', map_location=device))
model.eval()

with open('saved-models/scaler_humidity.pkl', 'rb') as f:
...
def test_historical_date(date_str):
    ...
    humidities = np.array(data['relative_humidity_2m'])

    # Extract input window
    past_mask = (times > reference_time - pd.Timedelta(hours=WINDOW_SIZE)) & (times <= reference_time)
    window_humidities = humidities[past_mask]
    if len(window_humidities) != WINDOW_SIZE:
        raise RuntimeError(f"Input window missing data: {len(window_humidities)} of {WINDOW_SIZE} points")

    # True future temperatures
    future_mask = (times > reference_time) & (times <= reference_time + pd.Timedelta(hours=PREDICT_HORIZON))
    true_humidities  = humidities[future_mask]
    future_times = times[future_mask]

    # LSTM iterative forecasting
    scaled_window = scaler.transform(window_humidities.reshape(-1, 1)).flatten()
    ...
    length = min(len(true_humidities), len(preds))
    true_humidities = true_humidities[:length]
    preds = preds[:length]
    valid = ~np.isnan(true_humidities) & ~np.isnan(preds)
    true_humidities = true_humidities[valid]
    preds = preds[valid]
    plot_times = future_times[:length][valid]

    mse = mean_squared_error(true_humidities, preds)
    print(f"\n=== Historical test for {date_str} ===")
    print(f"Data points used: {len(true_humidities)}, MSE: {mse:.4f}")

    plt.figure(figsize=(10, 5))
    plt.plot(plot_times, true_humidities, marker='o', label='Open-Meteo historical data')
...
def test_for_date(date_str):
    ...
    humidities_past = np.array(past_data['relative_humidity_2m'])
    mask = (times_past > reference_time - pd.Timedelta(hours=WINDOW_SIZE)) & (times_past <= reference_time)
    window_humidities = humidities_past[mask]
    assert len(window_humidities) == WINDOW_SIZE, f"Expected {WINDOW_SIZE} points, got {len(window_humidities)}"
    ...
    humidities_fut = np.array(future_data['relative_humidity_2m'])
    mask_fut = (times_fut > reference_time) & (times_fut <= reference_time + pd.Timedelta(hours=PREDICT_HORIZON))
    true_humidities = humidities_fut[mask_fut]
    ...
    plt.figure(figsize=(10, 5))
    plt.plot(forecast_times, true_humidities, marker='o', label='Open-Meteo forecast')
